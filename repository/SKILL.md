...
2. **Type your repositories.** Parametrize `BaseRepository[T]` with the entity type (e.g. `BaseRepository[User]`) so callers get typed returns instead of raw dicts.
3. **One repository per aggregate/entity.** Each concrete repository only sets its `table` name and adds entity-specific queries.
4. **Put shared CRUD in `BaseRepository`.** Only add methods to a concrete repository when the query is specific to that entity (e.g. `get_by_email`).
5. **Keep method names consistent:** `create`, `get`, `update`, `delete`, `list`.
6. **Inject the adapter** through the constructor. Never instantiate it inside the repository.
